...
class Qwen3Adapter(BaseModelAdapter):
    """The model adapter for Qwen/Qwen3-8B and other Qwen3 chat models.

    Qwen3 uses the same ChatML-style template as Qwen1/Qwen2
    (``<|im_start|>...<|im_end|>``), so we only need a dedicated adapter to
    select the ``qwen3`` conversation template for paths containing "qwen3".
    """
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        return get_conv_template("qwen3")
