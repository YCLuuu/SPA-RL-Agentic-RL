...
save_path=exploration/virtualhome/exploration_outputs/
logs_path=${save_path}logs
save_dir=ckt/
cur_model_name=llama3b_virtualhome_sft
worker_num=32
sample_node_num=4
task=virtualhome
...
explore_model_name=${cur_model_name}-explore-max40
...
for ((j=0;j<${sample_num_workers};j=j+1)); do
    ...
    CUDA_VISIBLE_DEVICES=${gpu_nodes[$j]} python -u -m fastchat.serve.vllm_worker \
        ...
        --port ${fs_worker_port} \
        --worker-address http://0.0.0.0:${fs_worker_port} >> ${logs_path}/model_worker-${j}.log 2>&1 &
...
step_traj_save_path=exploration/virtualhome/exploration_outputs/explore
...
for (( j = 0; j < $worker_num; j++ )); do
    python3 exploration/virtualhome/generate_response.py --agent_config fastchat_explore --iteration_num 10 --exp_config ${task} --model_name ${explore_model_name}-$((j%sample_node_num)) --part_num $((worker_num)) --part_idx ${j} --save_path ${step_traj_save_path}  >> ${logs_path}/gen_response_worker-${j}.log 2>&1 &
