...
class State:
    ...
    def __init__(
        ...
        self.steps = 0
        # Grounding signal bookkeeping (SPA-RL extension)
        self.grounding_checks: List[Dict[str, Any]] = []
    ...
    def load_json(cls, json_dict: Dict[str, Any]):
        ...
        state.grounding_checks = info.get("grounding_checks", [])
        state.invalid_action_count = info.get("invalid_action_count", 0)
        state.step_times = info.get("step_times", [])
        state.total_time = info.get("total_time", 0.0)
        return state
    ...
    def to_dict(self, format="fastchat") -> Dict[str, Any]:
        ...
        meta_info = {
            ...
            "error": self.error,
            "grounding_checks": self.grounding_checks,
